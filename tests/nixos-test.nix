...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    machine.succeed("command -v vm-attach")
    machine.succeed("vm-list | grep -qx test-vm")
    machine.fail("vm-attach unmanaged-vm")
    machine.succeed("tmux -L vm-attach-test new-session -d -s client 'vm-attach test-vm'")
    machine.wait_until_succeeds("test $(tmux -L vm-attach-test display-message -p -t client:0.0 '#{pane_current_command}') = tmux")
    machine.succeed("tmux -L vm-attach-test kill-server")
    machine.succeed("tmux -S /run/nixos-shell/test-vm.tmux has-session -t vm")

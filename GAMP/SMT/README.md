...
| FS-070-HDS-010-SDS-010-SMS-010 | OK | module and systemd integration tests; `restartOnGuestShutdown` defaults on, unexpected supervisor termination is recovered with a new service process, authorized guest shutdown hands recovery to a fresh supervisor using the loaded unit configuration, cleanup without a live `MAINPID` does not fabricate stop authority, and explicit stop plus carrier-down remain inactive beyond the service restart delay |
